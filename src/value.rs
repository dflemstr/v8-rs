...
impl ArrayBuffer {
    pub fn new(isolate: &isolate::Isolate, byte_length: usize) -> ArrayBuffer {
        let raw = unsafe {
            util::invoke(&isolate,
                         |c| v8::ArrayBuffer_New(c, isolate.as_raw(), byte_length as i32))
                .unwrap()
        };
    ...
    pub fn as_raw(&self) -> v8::ArrayBufferRef {
        self.1
    }
}
...
impl ArrayBufferView {
    pub fn buffer(&self) -> ArrayBuffer {
        let raw =
            unsafe { util::invoke(&self.0, |c| v8::ArrayBufferView_Buffer(c, self.1)).unwrap() };
        ArrayBuffer(self.0.clone(), raw)
    }

    pub fn byte_offset(&self) -> usize {
        unsafe {
            util::invoke(&self.0, |c| v8::ArrayBufferView_ByteOffset(c, self.1)).unwrap() as usize
        }
    }

    pub fn byte_length(&self) -> usize {
        unsafe {
            util::invoke(&self.0, |c| v8::ArrayBufferView_ByteLength(c, self.1)).unwrap() as usize
        }
    }
    ...
    /// Creates an array buffer view from a set of raw pointers.
    pub unsafe fn from_raw(isolate: &isolate::Isolate,
                           raw: v8::ArrayBufferViewRef)
                           -> ArrayBufferView {
        ArrayBufferView(isolate.clone(), raw)
    }

    /// Returns the underlying raw pointer behind this array buffer.
    pub fn as_raw(&self) -> v8::ArrayBufferViewRef {
        self.1
    }
}
...
impl Uint8Array {
    pub fn new(isolate: &isolate::Isolate,
        ...
        Uint8Array(isolate.clone(), raw)
    }

    /// Creates an uint8 array from a set of raw pointers.
    pub unsafe fn from_raw(isolate: &isolate::Isolate, raw: v8::Uint8ArrayRef) -> Uint8Array {
        Uint8Array(isolate.clone(), raw)
    }

    /// Returns the underlying raw pointer behind this uint8 array.
    pub fn as_raw(&self) -> v8::Uint8ArrayRef {
        self.1
    }
}
